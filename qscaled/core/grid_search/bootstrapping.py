...
def get_bootstrap_optimal(group):
    ...
    optimal_indices_bootstrap = np.argmin(
        times_bootstrap, axis=0
    )  # 100 indices of optimal batch sizes
    ...
    best_bs_bootstrap = batch_sizes[
        optimal_indices_bootstrap
    ]  # for each bootstrap sample, get the optimal batch size
...
def compute_bootstrap_averages(df_best_lr, df_best_bs, df_best_lr_bs):
    ...
    for env in get_envs(df_best_lr_bs):
        ...
        best_lr_bootstrap = np.stack(
            [np.stack(g['best_lr_bootstrap'].values) for _, g in utd_groups]
        )
...
def plot_bootstrap_average_params(df_best_lr_bs):
    ...
    def helper(optim_key, other_key, title):
        ...
        for i, env in enumerate(envs):
            ...
            point_corr = np.corrcoef(
                np.log10(env_data['utd']), np.log10(env_data[f'best_{optim_key}'])
            )[0, 1]
            axes[i].plot(
                env_data['utd'],
            ...
            mean_corr = np.corrcoef(
                np.log10(env_data['utd']), np.log10(env_data[f'best_{optim_key}_{other_key}mean'])
            )[0, 1]
            axes[i].errorbar(
                env_data['utd'],
            ...
            mean_bootstrap_corr = np.corrcoef(
                np.log10(env_data['utd']),
                np.log10(env_data[f'best_{optim_key}_bootstrap_{other_key}mean']),
            )[0, 1]
            axes[i].errorbar(
                env_data['utd'],
